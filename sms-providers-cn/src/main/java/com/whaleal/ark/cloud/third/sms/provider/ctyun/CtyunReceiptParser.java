 ...
 * @since 2026-07-29
 */
public class CtyunReceiptParser implements ReceiptParser {
    ...
    public SmsReceipt parse(Map<String, Object> rawData, SmsProviderConfig config) {
        ...
        String status = first(rawData, new String[]{"status", "result"});
        return SmsReceipt.builder()
                .receiptId(first(rawData, new String[]{"smsMsgId", "msgId"}))
                .messageId(first(rawData, new String[]{"smsMsgId", "msgId"}))
                .to(first(rawData, new String[]{"phone", "mobile"}))
                ...
                .providerType(SmsProviderType.CTYUN)
                .rawData(rawData)
                .build();
    }
