 ...
 * @since 1.0.0
 */
public class HuaweiReceiptParser implements ReceiptParser {
    
    private static final Logger logger = LoggerFactory.getLogger(HuaweiReceiptParser.class);
    private static final DateTimeFormatter DATE_FORMATTER = DateTimeFormatter.ISO_DATE_TIME;
    
    @Override
    public SmsReceipt parse(Map<String, Object> rawData, SmsProviderConfig config) {
        if (!isValidData(rawData)) {
            logger.warn("收到无效的华为云短信回执数据: {}", rawData);
            return null;
        }
        ...
        return SmsReceipt.builder()
            ...
            logger.error("解析华为云短信回执数据失败: {}", e.getMessage(), e);
            return null;
        }
    }
